...
export function validateObservation(observation, providerType = "sportsbook") {
  ...
  if (providerType === "multiplier") {
    ...
    if (!["passing_yards", "passing_touchdowns", "rushing_yards", "receiving_yards", "receptions", "offensive_touchdowns"].includes(observation.statType)) errors.push("multiplier market statType is unsupported");
    const hasHigher = observation.higherMultiplier !== undefined;
    const hasLower = observation.lowerMultiplier !== undefined;
    if (hasHigher !== hasLower) errors.push("Higher and Lower multipliers must be supplied together");
    if (hasHigher && !validateMultiplier(observation.higherMultiplier)) errors.push("higherMultiplier must be valid");
    if (hasLower && !validateMultiplier(observation.lowerMultiplier)) errors.push("lowerMultiplier must be valid");
  }
