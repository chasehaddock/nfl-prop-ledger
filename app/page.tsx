...
const SOURCE_NAMES: Record<string, string> = { fanduel: "FanDuel", prizepicks: "PrizePicks", underdog: "Underdog" };
...
const DEMO_LINES: PlayerLine[] = [
  demo("Joe Burrow", "CIN", "QB", "PrizePicks", 4275.5, 2.5, 31.5),
  demo("Saquon Barkley", "PHI", "RB", "PrizePicks", 1225.5, 46.5, 11.5),
  demo("Ja'Marr Chase", "CIN", "WR", "PrizePicks", 1375.5, 101.5, 10.5),
  demo("Brock Bowers", "LV", "TE", "PrizePicks", 1025.5, 91.5, 6.5),
];
...
function LineHistoryChart({ line, statType, history, movements, highlightedSource }: { line: PlayerLine; statType: StatType; history: History; movements: TrendMove[]; highlightedSource?: string }) {
  ...
  return <section className="line-history-chart" aria-labelledby={`chart-title-${line.id}-${statType}`}>
    ...
    <div className="line-chart-scroll"><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={summary}>
      <title>{line.player} · {STAT_LABELS[statType]} line history</title><desc>{summary} FanDuel is blue, PrizePicks is purple, and Underdog is yellow.</desc>
...
export default function Home() {
  ...
  const trendMoves = useMemo(() => {
    ...
    const allMoves = sportsbookAverageMovements(allLines, history, rawMoves, { includePrizePicks: true, minimumPairs: 1 });
  ...
  }, [snapshot, allLines, history, trendRange]);
  ...
  return <main>
    ...
    <section className="ledger" aria-labelledby="ledger-title">
      ...
      <div className="table-wrap">
        ...
        {lines.length === 0 && <div className={`empty-state ${waitingForWeekly ? "weekly-waiting" : ""}`}>{waitingForWeekly ? <><strong>Week 1 board is ready.</strong><span>Player projections will appear after the first confirmed Week 1 markets are captured from FanDuel, PrizePicks, or Underdog.</span></> : "No players match these filters."}</div>}
      </div>
    </section>
    </div>
